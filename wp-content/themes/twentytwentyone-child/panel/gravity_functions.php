...
function add_back_button_to_entry_detail($form, $entry) {
    if (in_array($form['id'], array(4, 5, 12, 15,30,39))) {
        $back_urls = [
        ...
        } elseif (in_array($form_id, [15, 30,39]) ) {
...
function exam_handle_approve_entry_ajax() {
  ...
  if ($entry['form_id'] == 30) {
   ...
   // $form_type = ($entry['form_id'] == 30) ? 'Retest' : 'Initial';
    if ($entry['form_id'] == 15) {
         $form_type = 'Initial';
        } elseif ($entry['form_id'] == 30) {
            $form_type = 'Retest';
        } elseif ($entry['form_id'] == 39) {
            $form_type = 'Renewal/Recertification';
        } else {
            $form_type = ' ';
        }

...
function display_form_entries_page($form_id, $is_retest = false) {
    ...
    // Server-side pagination and filtering based on user role
    $current_page = isset($_GET['paged']) ? max(1, intval($_GET['paged'])) : 1;
    $rows_per_page = 10;

    // User access control - get this BEFORE loading entries
    $current_user = wp_get_current_user();
    ...
    $is_manager_admin = current_user_can('manager_admin');
    ...
    // Set up search criteria based on user role
    $search_criteria = [
        'start_date' => null,
        'end_date' => null,
    ];

    // Only administrators load ALL entries
    if ($is_super_admin) {
        // Load all entries for administrators (no center restriction)
        $sorting = array(
            'key'       => 'date_created',
            'direction' => 'DESC'
        );
        ...
        $paging = array(
            'offset'    => 0,
            'page_size' => 999999  // Very high number to get all entries for admins
        );

        $entries = GFAPI::get_entries($form_id, $search_criteria, $sorting, $paging);
    } else {
        // For non-admin users, filter by their allowed centers using server-side filtering
        if (!empty($allowed_centers)) {
            // Add center filter to search criteria for non-admin users
            $search_criteria['field_filters'] = [
                [
                    'key' => $config['prefer_center'],
                    'operator' => 'IN',
                    'value' => $allowed_centers
                ]
            ];
        } else {
            // No centers assigned - return empty results
            $entries = [];
        }
        ...
        // Use normal pagination for non-admin users since they're already filtered
        $paging = array(
            'offset'    => 0,
            'page_size' => 999999  // Still load all their entries for client-side filtering
        );
    ...
    if (is_wp_error($entries)) {
        echo '<p class="text-red-600">Error loading entries: ' . esc_html($entries->get_error_message()) . '</p>';
        return;
    }
    ...
    // Permission check: Only restrict if user is not admin and has no center access
    if (!$is_super_admin && empty($allowed_centers)) {
        echo '<p class="text-red-600">You do not have permission to view this page or no centers are assigned to your account.</p>';
        return;
    }
    ...
    // Display access control information for debugging
    echo '<div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">';
    if ($is_super_admin) {
        echo '<p class="text-sm text-gray-700 mb-1"><strong>Access Level:</strong> Super Admin - Can view all entries</p>';
        echo '<p class="text-sm text-gray-700"><strong>Total entries in database:</strong> ' . esc_html(count($entries)) . '</p>';
    } else {
        $role_name = $is_manager_admin ? 'Manager Admin' : ($is_aqb_admin ? 'AQB Admin' : 'Center Admin');
        echo '<p class="text-sm text-gray-700 mb-1"><strong>Access Level:</strong> ' . $role_name . ' - Restricted to assigned centers only</p>';
        echo '<p class="text-sm text-gray-700 mb-1"><strong>Your Allowed Centers:</strong> ' . (empty($allowed_centers) ? '<span class="text-red-600">None assigned</span>' : implode(', ', $allowed_centers)) . '</p>';
        echo '<p class="text-sm text-gray-700"><strong>Total entries in database:</strong> ' . esc_html(count($entries)) . '</p>';
    }
    echo '</div>';
    ...
    echo '</select>';
    echo '</div>';
    if ($is_super_admin || $is_manager_admin) {
        echo '<div class="form_controls flex-1 min-w-[150px]">';
        ...
        echo '</select>';
        echo '</div>';
    } elseif (!empty($allowed_centers)) {
        // For AQB Admin and Center Admin, only show their allowed centers
        echo '<div class="form_controls flex-1 min-w-[150px]">';
        echo '<select id="centerFilter" class="border p-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">';
        echo '<option value="">All My Centers</option>';
        foreach ($allowed_centers as $center_name) {
            $selected = $center_name === $center_filter ? 'selected' : '';
            echo '<option value="' . esc_attr($center_name) . '" ' . $selected . '>' . esc_html($center_name) . '</option>';
        }
        echo '</select>';
        echo '</div>';
    }
    ...
    $displayed_count = 0; // Counter for entries that pass access control
    ...
    foreach ($entries as $index => $entry) {
        ...
        // Apply client-side filters (center, status, method, name)
        // Note: Server-side filtering already applied for non-admin users

        // Apply center filter if set (for all users including admins)
        if ($center_filter && $prefer_center !== $center_filter) {
            continue;
        }

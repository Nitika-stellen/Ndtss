<?php
/**
 * Membership Module Error Logger
 * Dedicated logging system for membership operations
 */
...
class MembershipLogger {
    ...
    public static function init() {
        self::$log_dir = get_stylesheet_directory() . '/membership/logs';
        self::$log_file = self::$log_dir . '/membership-errors-' . date('Y-m') . '.log';
    ...
    private static function log($level, $message, $context = []) {
        ...
        if (file_put_contents(self::$log_file, $log_entry, FILE_APPEND | LOCK_EX) === false) {
            ...
            error_log("Membership Logger: Failed to write to log file: " . self::$log_file);
        }
    ...
    private static function rotate_logs() {
        ...
        if ($file_size > self::$max_log_size) {
            // Archive current log
            $archive_file = self::$log_dir . '/membership-errors-' . date('Y-m') . '-' . time() . '.log';
    ...
    private static function cleanup_old_logs() {
        $log_files = glob(self::$log_dir . '/membership-errors-*.log');
...
// Initialize logger
MembershipLogger::init();

/**
 * Convenience functions for easy logging
 */
function membership_log_error($message, $context = []) {
    MembershipLogger::error($message, $context);
}

function membership_log_warning($message, $context = []) {
    MembershipLogger::warning($message, $context);
}

function membership_log_info($message, $context = []) {
    MembershipLogger::info($message, $context);
}

function membership_log_debug($message, $context = []) {
    MembershipLogger::debug($message, $context);
}

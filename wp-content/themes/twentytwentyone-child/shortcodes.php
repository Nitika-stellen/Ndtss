...
function load_cpd_form_shortcode() {
    ...
    if ($renew_method === 'cpd') {
        $form_id = 31; // CPD Form ID
    } elseif ($renew_method === 'exam') {
        $form_id = 31; // Exam Renewal Form ID
    } else {
        return '<p style="color:red;">Invalid or missing renewal method.</p>';
    }

    // Ensure Gravity Forms scripts are enqueued
    if (class_exists('GFForms')) {
        // Enqueue required scripts for Form 31
        gravity_form_enqueue_scripts($form_id, false);
        wp_enqueue_script('gform_conditional_logic');
        wp_enqueue_script('gform_gravityforms');
    }

    // Disable AJAX to prevent submission conflicts and use simple form loading
    // This prevents the "Another submission is already in progress" error
    return do_shortcode('[gravityform id="' . intval($form_id) . '" title="false" description="false" ajax="false"]');
}

   ...
   if (!is_user_logged_in()) {
    ...
    if (isset($_GET['entry_id'])) {
       ...
       // echo "<pre>";print_r($entry_data);
        ...
        //$form_id = isset($_GET['form_id']) && in_array($_GET['form_id'], ['30', '31', '15']) ? intval($_GET['form_id']) : 15;
       // echo $form_id;
        $config = $field_map['form_' . $entry_data['form_id']];
       ...
       ;
        ...
        <?php else: ?>
            ...
            if ($entry_data['form_id'] == 30 || $entry_data['form_id'] == 39) {
                $method_value = trim($entry_data['1'] ?? '');
                $methods = !empty($method_value) ? [$method_value] : [];
            } elseif ($entry_data['form_id'] == 15) {
                $methods = array_filter([
                    $entry_data['188.1'], $entry_data['188.2'], $entry_data['188.3'],
                    $entry_data['188.4'], $entry_data['188.5'], $entry_data['188.6'],
                    $entry_data['188.7'], $entry_data['188.8'], $entry_data['188.9']
        ...
        if ($entry_data['form_id'] == 30) { $matched_entry = $entries[0]; // Take the first match

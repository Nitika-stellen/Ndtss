...
if (!empty($signature_path) && file_exists($signature_path)) {
    ...
    $method_code = strtoupper(substr($method, 0, 3));
    $year = date('y', current_time('timestamp')); // Uses WordPress time setting
    $serial = str_pad($marks_entry_id % 1000, 3, '0', STR_PAD_LEFT);
    $certificate_number = $candidate_reg_number . '-' . strtoupper($method_slug);
    ...
    // $certificate_number = $candidate_reg_number . '-' . strtoupper($method_slug);
    $certificate_number = $candidate_reg_number . '01';
    ...
    <div style="position:relative; font-size:11pt;">
        ...
        <div style="text-align:center; margin-top:10px;">
            ...
            <table style="width:100%; border-collapse:collapse; text-align: center; margin-top:30px; border-color: #bdbdbd;" border="1" cellpadding="4">
                ...
                <tbody><tr style="text-align: center;"><td style="color: #1712fd;">' . $method . '</td><td style="color: #1712fd;">' . $certificate_number . '-1</td><td style="color: #1712fd;">' . $sector . '</td><td style="color: #1712fd;">' . $exam_level . '</td><td style="color: #1712fd;">' . (!empty($scope) && is_array($scope) ? implode(', ', $scope) : '') . '</td><td style="color: #1712fd;">' . $issue_date . '</td><td style="color: #1712fd;">' . $expiry_date . '</td></tr></tbody>
            </table>

...
if ( !function_exists( 'child_theme_configurator_css' ) ):
	function child_theme_enqueue_frontend_styles() {
		...
		wp_enqueue_style(
			'frontend-css',
			get_stylesheet_directory_uri() . '/css/frontend-style.css',
			array(),
			filemtime(get_stylesheet_directory() . '/css/frontend-style.css')
		);
	}
endif;
...
function populate_user_data_into_form($form) {
	...
	foreach ($form['fields'] as &$field) {
		...
		switch ($field->id) {
          	case 1: $field->defaultValue = get_user_meta($user_id, 'family_name', true); break;
			case 840:$field->defaultValue = get_user_meta($user_id, 'first_name', true);break;
			case 841:$field->defaultValue = get_user_meta($user_id, 'last_name', true);break;
			case 835:$field->defaultValue = get_user_meta($user_id, 'national_id', true);break;
			case 3: $field->defaultValue = get_user_meta($user_id, 'dob', true); break;
			case 860:
				$id_proof_url = get_user_meta($user_id, 'custom_id_proof', true);

				if ($id_proof_url) {
					// Show preview of the uploaded ID Proof
					$field->description .= '<div class="QR_proof_form">
						<div class="QR_proof_form_detail">
						<strong>Current ID Proof:</strong><br>
						<p style="font-size: 0.9em; color: #666;">Upload a new ID proof only if you want to replace this one.</p>
						</div>
						<a href="' . esc_url($id_proof_url) . '" target="_blank" style="display:inline-block; margin-top:5px;">
							<img src="' . esc_url($id_proof_url) . '" alt="ID Proof" style="max-width:150px; border:1px solid #ccc; padding:5px;">
						</a>
					</div>';
			...
			case 861:
				...
				if ($photo_url) {
					// Show preview in the description
					$field->description .= '<div class="QR_proof_form">
					<div class="QR_proof_form_detail">
					<strong>Current Profile Photo:</strong>
					<p style="font-size: 0.9em; color: #666;">Please upload 1 passport size photo</p>
					</div>
					
						<img src="' . esc_url($photo_url) . '" style="max-width:150px; border:1px solid #ccc; padding:5px;">
						
					</div>';

					// ✅ Make the field NOT required if a photo already exists
					$field->isRequired = false;
				}
				break;

			case 839:if (in_array($saved_prefix, $standard_prefixes)) {
					$field->defaultValue = $saved_prefix;
				} else {
					$field->defaultValue = 'Other';
				}
				break;
			case 4:if (!in_array($saved_prefix, $standard_prefixes)) {
					$field->defaultValue = $saved_prefix;
				}
				break;
...
function send_exam_submission_notification($entry_id, $order_number, $center_post) {
    ...
    // -----------------------------
    // Get center admins (handles multiple IDs)
    // -----------------------------
    $center_admin_ids = get_post_meta($center_post->ID, '_center_admin_id', true);
    $center_admin_ids = maybe_unserialize($center_admin_ids);

    $admin_emails = [];

    if (!empty($center_admin_ids)) {
        if (is_array($center_admin_ids)) {
            foreach ($center_admin_ids as $admin_id) {
            ...
            // single admin case
            $user = get_userdata($center_admin_ids);
            if ($user && is_email($user->user_email)) {
                $admin_emails[] = $user->user_email;
            }
        }
    ...
    // Get all site admins
    // -----------------------------
    $site_admins = get_users([
        'role'   => 'administrator',
        'fields' => ['user_email'],
    ]);

    foreach ($site_admins as $site_admin) {
        if (is_email($site_admin->user_email) && !in_array($site_admin->user_email, $admin_emails)) {
            $admin_emails[] = $site_admin->user_email;
        }
    }
    ...
    // --- 2. Send mail to center admin(s) + all admins ---
    error_log('Admin emails prepared: ' . print_r($admin_emails, true));
    if (!empty($admin_emails)) {
        $admin_subject = '📢 New Exam Form Submission Received';
        $admin_message  = '<p><strong>Exam Form Submission Notification</strong></p>';
        $admin_message .= '<p>A new exam form has been submitted by <strong>' . esc_html($candidate_name) . '</strong>.</p>';
...
function remove_exam_center_add_new_menu() {
	if (!current_user_can('administrator') && get_current_screen()->post_type === 'exam_center') {
...
function examiner_entry_response() {   
	...
	if ($status === 'declined') {
		...
		$body = "<p><strong>{$user->display_name}</strong> has declned the examiner assignment.</p>";
		$body .= "<p><strong>Order:</strong> #{$order_number}</p>";
		$body .= "<p><strong>Center:</strong> " . esc_html($center_name) . "</p>";
		$body .= "<p><strong>Comment:</strong> " . esc_html($comment) . "</p>";
		$message = get_email_template($subject, $body);
...
add_filter('gform_pre_render_36', 'populate_user_data');
add_filter('gform_pre_validation_36', 'populate_user_data');
add_filter('gform_pre_submission_filter_36', 'populate_user_data');
add_filter('gform_admin_pre_render_36', 'populate_user_data');
...
function ul_render_membership_section() {
    $current_user = wp_get_current_user();
    ...
    ?>
    <!-- Membership Section -->
    <div id="membership-sectionxc" class="user-profile-tab-content" style="display:none;" role="tabpanel" aria-labelledby="membership-tab">
...
// Shortcode to render membership section in frontend
function ul_membership_section_shortcode() {
    if (!is_user_logged_in()) {
        return '<p>Please log in to view your membership details.</p>';
    }
    return ul_render_membership_section();
}
add_shortcode('membership_section', 'ul_membership_section_shortcode');

// Add certificate section to user profile (admin)
function ul_add_certificate_to_user_profile($user) {
    // Only show to admins or the user themselves
    if (current_user_can('manage_options') || get_current_user_id() == $user->ID) {
        $membership_data = get_user_meta($user->ID, 'membership_data', true);
        $membership_data = is_array($membership_data) ? $membership_data : [];
        ?>
                    ...
                    <?php if (!empty($membership_data)) : ?>
                        <table class="widefat striped">
                            ...
                            <tbody>
                                <?php foreach ($membership_data as $membership) : ?>
                                    <tr>
                                        <td><?php echo esc_html($membership['membership_number'] ?? 'N/A'); ?></td>
                                        <td><?php echo esc_html($membership['membership_type'] ?? 'N/A'); ?></td>
                                        <td>
                                            <?php if (!empty($membership['certificate_url'])) : ?>
                                                <a href="<?php echo esc_url($membership['certificate_url']); ?>" class="button" download>Download Certificate</a>
                                            <?php else : ?>
                                        ...
                                        <td><?php echo esc_html($membership['expiry_date'] ? date('d/m/Y', strtotime($membership['expiry_date'])) : 'N/A'); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
...
add_action('show_user_profile', 'ul_add_certificate_to_user_profile');
add_action('edit_user_profile', 'ul_add_certificate_to_user_profile');
